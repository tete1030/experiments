    ...
    try:
        ...
        if resume_run_id is None:
            if isinstance(exit_exception, ExitWithDelete) or \
                    ...
                    not ask("Save run and checkpoint ({})?".format(globalvars.main_context.run_id), posstr="y", negstr="trash", ansretry=2, ansdefault=True, timeout_sec=60 if caught_exception is None else None):
                do_trash = True
