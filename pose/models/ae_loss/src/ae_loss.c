...
#include "stdio.h"
#include "stdlib.h"
#include "ae_loss_kernel.h"

#define real float
extern THCState *state;

int ae_loss_forward(THCudaTensor *Tag, THLongTensor *keypoints, THFloatTensor *output, THFloatTensor *mean_tags){
    //Tag: batch x (17*h*w) x #tag_dim(1)
    //keypoints: #batch x max_num_person x 17 x (flatten pos, flag)
    //ouptut: #batch x (inter-person loss, intra-person loss)
    //mean_tags: #batch x max_num_person x (1+1)
