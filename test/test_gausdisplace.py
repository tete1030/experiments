...
argparser.add_argument("--point-ksize", type=int)
argparser.add_argument("--point-sigma", type=float)
argparser.add_argument("--sampler", choices=["gaussian", "uniform"], default="gaussian")
...
POINT_KSIZE = args.point_ksize if args.point_ksize is not None else 3
POINT_SIGMA = args.point_sigma if args.point_sigma is not None else 1.
...
displacer = PositionalGaussianDisplaceModule(NUM_CHANNEL, NUM_SAMPLE, SIGMA_ANGLE / 180 * np.pi, SIGMA_SCALE, sampler=args.sampler, fill=0).cuda()
