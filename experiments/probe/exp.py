...
class MainExperiment(BaseExperiment):
    def init(self):
        ...
        if self.offset_optimizer is not None:
            ...
            for dm in globalvars.displace_mods:
                if dm.offset is None or dm.offset.size(0) == 0:
                    continue
    ...
    def init_model(self):
        ...
        assert globalvars.myblock_counter == len(hparams.MODEL.LEARNABLE_OFFSET.EXPAND_CHAN_RATIO) or not hparams.MODEL.DETAIL.ENABLE_OFFSET_BLOCK
    ...
    def init_optimizer(self):
        ...
        if not hparams.MODEL.DETAIL.DISABLE_DISPLACE:
            self.offset_parameters = list(filter(lambda x: x.requires_grad,
                [dm.offset for dm in globalvars.displace_mods if hasattr(dm, "offset") and dm.offset is not None]))
            
            probe_modules = filter(lambda x: isinstance(x, Probe), self.model.modules())
            self.offset_parameters += list(filter(lambda x: x.requires_grad,
                [pm.offsets for pm in probe_modules]))
        ...
        if len(self.offset_parameters) > 0:
            arc_offset_parameters = list(filter(lambda x: x.requires_grad, [dm.offset for dm in globalvars.displace_mods if hasattr(dm, "offset") and dm.offset is not None and hasattr(dm, "arc_gaussian_displacer") and dm.arc_gaussian_displacer is not None]))
    ...
    def _save_offsets(self, step):
        ...
        if not offset_disabled:
            # TODO: Save probe offsets
            torch.save([(dm.get_all_offsets(detach=True) * dm.offset_scale).cpu() for dm in globalvars.displace_mods if not dm.runtime_offset], os.path.join(globalvars.main_context.checkpoint_dir, "offset_{}.pth".format(step)))
            if hparams.MODEL.LEARNABLE_OFFSET.DPOOL_SIZE > 1:
        ...
        if train and not hparams.MODEL.DETAIL.DISABLE_DISPLACE and self.offset_optimizer is not None and progress["step"] >= hparams.TRAIN.OFFSET.TRAIN_MIN_STEP:
            if tb_writer:
                ...
                for idm in range(len(globalvars.displace_mods)):
                    ...
                    if dm.offset is None or dm.offset.size(0) == 0:
                        continue
                    self.move_dis_avgmeter[iavgmeter].update((dm.offset.detach() * dm.offset_scale).cpu())
                    move_dis_avg.append(self.move_dis_avgmeter[iavgmeter].avg)
                    move_dis.append(self.move_dis_avgmeter[iavgmeter].lastdiff)
                    iavgmeter += 1
