...
from .offset import OffsetBlock, ActiveBlock
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, inshape_factor, res_index, block_index, stride=1, downsample=None):
        ...
        if not (self.res_index in [1, 2, 3] and self.block_index == 1) and (self.res_index != 2 or self.block_index < 6) and hparams.MODEL.DETAIL.ENABLE_OFFSET_BLOCK:
            expand_chan_ratio = hparams.MODEL.LEARNABLE_OFFSET.EXPAND_CHAN_RATIO[globalvars.myblock_counter]
            use_special = bool(hparams.MODEL.LEARNABLE_OFFSET.USE_SPECIAL[globalvars.myblock_counter] > 0)
            if expand_chan_ratio > 0:
                if hparams.MODEL.LEARNABLE_OFFSET.USE_ACTIVE_BLOCK:
                    self.offset_block = ActiveBlock(
                        hparams.MODEL.INP_SHAPE[1] // self.inshape_factor,
                        hparams.MODEL.INP_SHAPE[0] // self.inshape_factor,
                        self.inplanes,
                        self.inplanes,
                        int(self.inplanes * expand_chan_ratio // hparams.MODEL.ACTIVE_BLOCK.OFFSET_PER_CHANNEL),
