...
class ExtraMod(nn.Module):
    def __init__(self, width, height, inplanes, res_index, block_index):
        super(ExtraMod, self).__init__()
        self.width = width
        self.height = height
        ...
        self.displace = DisplaceChannel(
            ...
            hparams["model"]["detail"]["channels_per_pos"][res_index],
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, inshape_factor, res_index, block_index, stride=1, downsample=None):
        ...
        self.block_index = block_index
        if self.use_extra_mod:
            self.extra_mod = ExtraMod(hparams["model"]["inp_shape"][0] // self.inshape_factor,
                                      hparams["model"]["inp_shape"][1] // self.inshape_factor,
                                      self.inplanes, self.res_index, self.block_index)
            # TODO:
            Experiment.exp.extra_mods.append(self.extra_mod)
        else:
            self.extra_mod = None
