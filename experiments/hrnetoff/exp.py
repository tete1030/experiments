...
class GroupNormWrapper(nn.GroupNorm):
    def __init__(self, num_features, eps=1e-5, num_groups=32, momentum=None):
...
class Experiment(BaseExperiment):
    ...
    def init_model(self):
        ...
        assert not hparams.MODEL.DETAIL.FIRST_ESP_ONLY, "FIRST_ESP_ONLY not implemented for hrnet"
    ...
    def iter_process(self, epoch_ctx: EpochContext, batch: dict, progress: dict, train: bool) -> torch.Tensor:
        ...
        output_map, early_predictor_outputs = self.model(img)
...
class MainModel(nn.Module):
    def __init__(self, output_shape, num_points, pretrained=None):
        ...
        self.pose_net = get_pose_net(num_points, pretrained)

    def forward(self, x):
        prediction = self.pose_net(x)
        return prediction
