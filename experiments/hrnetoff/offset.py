    ...
    def __init__(self, inplanes, outplanes, input_shape=None, bias_planes=0, bias_factor=0, space_norm=True, stride=1):
        ...
        if space_norm:
            self.atten = nn.Sequential(
                ...
                nn.BatchNorm2d(outplanes, momentum=hparams.TRAIN.BN_MOMENTUM),
                nn.Softplus(),
                SpaceNormalization())
        else:
            self.atten = nn.Sequential(
...
class OffsetBlock(nn.Module):
    def __init__(self, height, width, inplanes, outplanes, displace_planes, stride=1, downsample=None):
        ...
        self.bn = nn.BatchNorm2d(self.outplanes, momentum=hparams.TRAIN.BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=True)
        if downsample is not None:
            self.downsample = downsample
        elif stride > 1 or inplanes != outplanes:
