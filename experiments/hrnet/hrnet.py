...
from utils.miscs import log_e, log_i
...
class PoseHighResolutionNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, offset_expands, stride=1):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                nn.BatchNorm2d(planes * block.expansion, momentum=hparams.TRAIN.BN_MOMENTUM),
            )

        layers = []
        layers.append(block(self.inplanes, planes, 1, offset_expands[0], stride=stride, downsample=downsample))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, 1, offset_expands[0]))
